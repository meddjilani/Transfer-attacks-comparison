    ...
    for epoch in range(nb_epoch):
        ...
        for i, (x_val, y_val) in enumerate(loader):
            if batch_delta.grad is not None:
                ...
                print('batch_delta.grad is None at iteration ',i)
            ...
            perturbed = torch.clamp((x_val + batch_delta).cuda(), 0, 1) if torch.cuda.is_available() else torch.clamp((x_val + batch_delta), 0, 1) 
            outputs = model(perturbed)
            
            # loss function value
            if layer_name is None:
                loss = clamped_loss(outputs, y_val.cuda()) if torch.cuda.is_available() else clamped_loss(outputs, y_val) 
            else:
                loss = main_value
