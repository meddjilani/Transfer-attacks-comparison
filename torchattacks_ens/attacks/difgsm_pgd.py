...
class DIFGSM_PGD(Attack):
    ...
    Examples::
        >>> attack = torchattacks.DIFGSM_PGD(model, eps=8/255, alpha=1/255, steps=10, resize_rate=0.9, diversity_prob=0.5, random_start=True)
        >>> adv_images = attack(images, labels)

    """
    def __init__(self, model, device = None, eps=8/255,
                 alpha=2/255, steps=10, resize_rate=0.9, diversity_prob=0.5, random_start=True):
        super().__init__("DIFGSM_PGD", model, device)
        self.eps = eps
        self.alpha = alpha
        self.steps = steps
