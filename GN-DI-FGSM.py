...
if __name__ == '__main__':
    ...
    parameters = {'attack': 'GN-DI-FGSM', **vars(args), **config}
    experiment.log_parameters(parameters)
    experiment.set_name("GN-DI-FGSM_"+args.model+"_"+args.target)
    ...
    for batch_ndx, (x_test, y_test) in enumerate(loader):
        ...
        print('Running GN-DI-FGSM attack on batch ', batch_ndx)
        ...
        adv_images_GN_DI = attack(x_test, y_test)


        acc = clean_accuracy(target_model, x_test, y_test)
        rob_acc = clean_accuracy(target_model, adv_images_GN_DI, y_test)
        ...
        suc_rate = 1 - clean_accuracy(target_model, adv_images_GN_DI[correct_batch_indices,:,:,:], y_test[correct_batch_indices])
