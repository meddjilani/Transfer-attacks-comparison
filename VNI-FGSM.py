...
if __name__ == '__main__':
    ...
    adv_images_VNI = attack(x_test_correct.to(device), y_test_correct.to(device))

    VNI_results = dict()
    for i,target_model in enumerate(target_models):
        acc = clean_accuracy(target_model, adv_images_VNI, y_test_correct)
        print('Robust Acc {}: %2.2f %%'%(acc*100))
        VNI_results[rank_target_models[i]] = acc
    ...
    data_VNIFGSM = data.get('VNI-FGSM',{})
    data_VNIFGSM.update(VNI_results)
    data['VNI-FGSM'] = data_VNIFGSM
