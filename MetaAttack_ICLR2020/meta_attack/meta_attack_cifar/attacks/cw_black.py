...
class BlackBoxL2:
    ...
    def _optimize(self, model, meta_model, step, input_var, modifier_var, target_var, scale_const_var, target, batch_idx, indice,input_orig = None):
        ...
        # print("input var: ",input_var.min().item(),input_var.max().item())#print('output shape', model(input_adv).shape)
        ...
        #print()#pdb.set_trace()
        input_diff = torch.clamp(input_adv - input_orig,min= -self.eps, max= self.eps)
        input_adv = torch.clamp(input_orig + input_diff, min=0, max=1)
