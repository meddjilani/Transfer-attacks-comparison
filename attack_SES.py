...
def main():
    ...
    parameters = {'attack': 'SES', **vars(args), "targeted": False if args.untargeted else True,
    ...
    for im_idx, batch in tqdm(enumerate(testloader)):
        ...
        adv_np, losses = get_adv_np(im_np, tgt_label, w_np, wb, bound, eps, n_iters, alpha, resize_rate, diversity_prob,
                                    algo=algo, fuse=fuse, untargeted=args.untargeted, loss_name=loss_name,
                                    adv_init=None)
        ...
        if (not args.untargeted and label_idx == tgt_label) or (args.untargeted and label_idx != tgt_label):
            ...
            while n_query < args.iterw:
                ...
                adv_np_plus, losses_plus = get_adv_np(im_np, tgt_label, w_np_temp_plus, wb, bound, eps, n_iters, alpha,
                                                      resize_rate, diversity_prob, algo=algo, fuse=fuse,
                ...
                adv_np_minus, losses_minus = get_adv_np(im_np, tgt_label, w_np_temp_minus, wb, bound, eps, n_iters,
                                                        alpha, resize_rate, diversity_prob, algo=algo, fuse=fuse,
                                                        untargeted=args.untargeted, loss_name=loss_name, adv_init=None)
    ...
    if (args.untargeted):
        print(
            f"untargeted. total_success: {len(success_idx_list)}; success_rate: {len(success_idx_list) / (im_idx + 1)},"
            f" avg queries: {np.mean(query_list)}")
    else:
        print(
            f"targeted. total_success: {len(success_idx_list)}; success_rate: {len(success_idx_list) / (im_idx + 1)},"
            f" avg queries: {np.mean(query_list)}")
