...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        random_range = 0.22 
        size_identity = x.size(1) if self.equalInOut else self.convShortcut(x).size(1)
        perturb_var = torch.rand((1, size_identity, 1, 1), requires_grad=False, device=x.device) * random_range * 2 + (
            1 - random_range)

        return torch.add(x*perturb_var if self.equalInOut else self.convShortcut(x)*perturb_var, out)
