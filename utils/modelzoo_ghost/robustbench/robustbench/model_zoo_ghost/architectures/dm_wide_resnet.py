...
class _Block(nn.Module):
    ...
    def forward(self, x):
        ...
        random_range = 0.22
        size_identity = self.shortcut(x).size(1) if self.has_shortcut else x.size(1)
        perturb_var = torch.rand((1, size_identity, 1, 1), requires_grad=False, device=x.device) * random_range * 2 + (
            1 - random_range)          
        out = torch.add(self.shortcut(x)*perturb_var if self.has_shortcut else x*perturb_var, out)
...
class _PreActBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return out + shortcut*perturb_var
