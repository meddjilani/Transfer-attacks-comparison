...
if __name__ == '__main__':
    ...
    netD.eval()
    print(device)
    original_net = robustbench.utils.load_model(model_name='Carmon2019Unlabeled', dataset='cifar10', threat_model='Linf')
    ...
    def get_att_results(model, target):
        ...
        for data in testloader:
            ...
            if target:
                ...
                acc_num += acc_sign.sum().float()
                print('inputs and labels',inputs.is_cuda, labels.is_cuda)
                print('original_net',next(original_net.parameters()).is_cuda )
    ...
    for epoch in range(opt.niter):
        ...
        if best_att < att_result:
            torch.save(netD.state_dict(),
                            opt.save_folder+'/resnet50_carmon_netD_epoch_%d.pth' % (epoch))
            torch.save(netG.state_dict(),
                            opt.save_folder+'/resnet50_carmon_netG_epoch_%d.pth' % (epoch))
            best_att = att_result
            print('model saved')
        else:
