    ...
    def run(self, model, meta_model, input, target, batch_idx = 0):
        ...
        for search_step in range(self.binary_search_steps):
            ...
            for step in range(self.max_steps):
                ...
                if step % 1 == 0 or step == self.max_steps - 1:
...
class BlackBoxLinf:
    ...
    def _optimize(self, model, meta_model, step, input_var, modifier_var, target_var, scale_const_var, target, batch_idx, indice,input_orig = None):
        ...
        #pdb.set_trace()
        if input_orig is None:
            dist = linf_dist(input_adv, input_var, keepdim = True).squeeze(2).squeeze(2)
        else:
            dist = linf_dist(input_adv, input_orig, keepdim = True).squeeze(2).squeeze(2)
    ...
    def run(self, model, meta_model, input, target, batch_idx = 0):
        ...
        # python/numpy placeholders for the overall best linf, label score, and adversarial image
        o_best_linf = [1e10] * batch_size
        o_best_score = [-1] * batch_size
        o_best_attack = input.permute(0, 2, 3, 1).cpu().numpy()
                ...
                for i in range(batch_size):
                    ...
                    if self.debug:
                        if step % 100 == 0:
                            print('{0:>2} dist: {1:.5f}, output: {2:>3}, {3:5.3}, target {4:>3}'.format(
                                i, di, output_label, output_logits[output_label], target_label))
                    if di < best_linf[i] and self._compare(output_logits, target_label):
                        if self.debug:
                            print('{0:>2} best step,  prev dist: {1:.5f}, new dist: {2:.5f}'.format(
                                  i, best_linf[i], di))
                        best_linf[i] = di
                        best_score[i] = output_label
                    if di < o_best_linf[i] and self._compare(output_logits, target_label):
                        if self.debug:
                            print('{0:>2} best total, prev dist: {1:.5f}, new dist: {2:.5f}'.format(
                                  i, o_best_linf[i], di))
                        o_best_linf[i] = di
