...
def main():
    ...
    for im_idx,(image,label) in tqdm(enumerate(testloader)):
        ...
        if label.item() == pred_vic:
            correct_pred +=1
        print(label.item(), ' ', torch.argmax(victim_model(image)).item(), ' ', label.item() == torch.argmax(victim_model(image)).item())
    ...
    metrics = {'robust_acc': rob_acc, 'suc_rate' : suc_rate, 'minmaxmeanqueries': np.mean(query_list)} #loss function
    experiment.log_metrics(metrics, step=im_idx)
