"""
baseline + update weights only if the target loss decreases + initialize the weight vector with the mean of the correctly classified images + adding  the exploitation andn exploration in the weight space
"""
...
def main():
    ...
    suc_rate = suc_adv / correct_pred
    metrics = {'robust_acc': rob_acc, 'suc_rate' : suc_rate, 'queries': np.mean(query_list)}
