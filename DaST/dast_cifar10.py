...
parser.add_argument('--source_model', type=str, default='Gowal2021Improving_28_10_ddpm_100m')
parser.add_argument('--model', type=str, default='Carmon2019Unlabeled')
parser.add_argument('--workers', type=int, help='number of data loading workers', default=2)
parser.add_argument('--batch_size', type=int, default=500, help='input batch size')
...
netD = load_model(model_name=opt.source_model, dataset='cifar10', threat_model='Linf')
netD.to(device)

original_net = load_model(model_name=opt.model, dataset='cifar10', threat_model='Linf')
...
testloader = torch.utils.data.DataLoader(testset, batch_size=opt.batch_size, sampler = sp.SubsetRandomSampler(data_list), shuffle=False, pin_memory=True) #testloader
...
pre_conv_block = []
for i in range (10):
    # pre_conv_block.append(nn.DataParallel(pre_conv(10).to(device)))
...
netG = Generator_cifar10(10).to(device)
...
for epoch in range(opt.niter):
    ...
    for ii in range(batch_num):
        ...
        # (1) Update D network:
        ###########################
        noise = torch.rand(opt.batch_size, nz, 1, 1, device=device)
