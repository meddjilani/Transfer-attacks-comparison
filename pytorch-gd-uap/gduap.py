...
def get_fooling_rate(model, delta, data_loader, device, experiment, disable_tqdm=False):
    """
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(data_loader, disable=disable_tqdm):
            ...
            total += images.size(0)
            flipped += (labels != adv_predicted).sum().item()
            if experiment is not None:
                changed = (labels != adv_predicted).sum().item()
                accurate = (labels == predicted).sum().item()
                acc = (accurate/images.size(0))*100
                rob_acc = (1-( changed/images.size(0)) ) * 100
                print('\nClean Acc: %2.2f %%'% acc)
                print('Robust Acc: %2.2f %%'% rob_acc)
...
def get_baseline_fooling_rate(model, device, experiment, disable_tqdm=False):
    """
    Baseline fooling rate is always evaluated on ILSVRC 2012 dataset
    """
    xi_min = -10/255
    xi_max = 10/255
    delta = (xi_min - xi_max) * torch.rand((1, 3, 32, 32), device=device) + xi_max
    delta.requires_grad = True
    data_loader = get_data_loader('imagenet')
    fr = get_fooling_rate(model, delta, data_loader, device, experiment, disable_tqdm=disable_tqdm)
    return fr
...
def gd_universal_adversarial_perturbation(model, model_name, train_type, batch_size, device, dataset_name, patience_interval, id,n_examples, max_iter, eps, disable_tqdm=False):
    """
    ...
    x_t, y_t = load_cifar10(n_examples=n_examples) if dataset_name=="cifar10" else None
    ...
    for i in tqdm(range(max_iter), disable=disable_tqdm):
        ...
        if iter_since_last_fooling > 400 or (sat_should_rescale and iter_since_last_fooling > 200):
            ...
            current_fooling_rate = get_fooling_rate(model, delta, data_loader, device, experiment = None,disable_tqdm=disable_tqdm)
            ...
            if current_fooling_rate > best_fooling_rate:
                ...
                filename = f"perturbations_uap/{id}_{model_name}_{train_type}_iter={i}_val={dataset_name}_fr={int(best_fooling_rate * 1000)}"
