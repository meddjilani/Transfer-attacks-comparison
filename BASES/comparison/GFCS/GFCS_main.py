...
parser.add_argument('--device', default='cpu', help='Device for evaluating networks.')
...
parser.add_argument('--n_examples', default=10, type=int, help='Number of sample images to attack.')
...
parser.add_argument("--linf", nargs="?", default=0.03, help="linf bound - 0.0625")
...
#                 'mobilenet_v3_small', 'wide_resnet50_2', 'efficientnet_b4', 'regnet_x_400mf'] # 'convnext_small','vit_b_16'
...
model = load_model(args.model_name, dataset='cifar10', threat_model='Linf')
model.to(device)
...
for model_name in surrogate_names:
    ...
    #         pretrained_model
    #     )
    surrogate_model_list.append(pretrained_model.to(device))
...
testloader = torch.utils.data.DataLoader(testset, batch_size = 1, sampler=torch.utils.data.sampler.SubsetRandomSampler(range(args.n_examples)), shuffle = False)
...
# for i, im_path in tqdm(enumerate(img_paths)):
    ...
    if args.targeted:
        ...
        label_attacked[0] = args.target_label
    ...
    if to_attack:
        ...
        for m in range(args.num_step):
            if args.ODS:
                ...
                if args.GFCS:
                    random_direction = torch.zeros(1, 10).to(device)
                    ...
                    if surrogate_ind_list.numel() > 0:
                        ...
                        random_direction = torch.rand((1, 10)).to(device) * 2 - 1
            ...
            if success:
                print('image %d: attack is successful. query = %d, dist = %.4f' % (
                        i + 1, nQuery, (X_best - image).norm(p=np.inf).item()))
                if args.GFCS:
        ...
        # print('X_best ',X_best.shape,X_best.min().item(),X_best.max().item())
        # print('image ',image.shape,image.min().item(),image.max().item())
        diff = torch.max(torch.abs(X_best[0,:,:,:] - image[0,:,:,:]))
        ...
        #print(im_np.transpose(1, 2, 0).shape)
        # diff = adv_np - im_np.reshape(32,32,3)
...
parameters = {'attack':'GFCS', 'sources':args.surrogate_names, 'target':args.model_name, 'n_examples':args.n_examples, 'ODS':args.ODS,
              'GFCS':args.GFCS, 'num_step':args.num_step, 'step_size':args.step_size,
            'norm_bound':args.norm_bound, 'linf':args.linf,
            'targeted':args.targeted, 'target_label':args.target_label
}
